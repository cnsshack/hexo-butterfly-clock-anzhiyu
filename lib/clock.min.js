...
function getIpInfo() {
    ...
    "true" === clock_default_rectangle_enable && e ? fetch(`https://restapi.amap.com/v3/geocode/regeo?key=${gaud_map_key}&location=${clock_rectangle}`).then((e => e.json())).then((e => {
        ...
        //增加qweather_host为和风天气host地址,去除“key=”改为和风api的key，增加请求标头验证 qweather_key
    )).then((c => {
            fetch(`https://${qweather_host}/v7/weather/now?location=${clock_rectangle}`,{
            ...
            e.qweather_url = `https://${qweatherHost}/v7/weather/now?location=${a}`
